...
function login()
{
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $query = "SELECT * FROM Adminstrator WHERE email ='$email'";
...
function createTransporter()
{
    ...
    if ($num_rows) {
        ...
        $modfname = strtoupper($fname);
        $modlname = strtoupper($lname);

        if (mysqli_query($conn, "INSERT INTO Transporter(fname, lname, email, phone, nrc, finger_id, status, password) VALUES('$modfname', '$modlname','$email', '$phone', '$nrc', '$finger_id', '$status', '$password');")) {
...
function assign_trunk()
{
    ...
    if (mysqli_num_rows($trans_query) > 0 && mysqli_num_rows($trans_query) > 0) {
        ...
        if ($trunk_query && $trans_query) {
            ...
            $transit_sql = "INSERT INTO `Transit`(depature, destination, cargo, trunk_id, transporter_id, state) VALUES('Lusaka', '$destination', '$cargo', '$trunk_id', '$transporter_id', 'Online')";
            ...
            if ($transit_result) {
                ...
                $count_sql = "SELECT transit_id FROM Transit WHERE trunk_id = $trunk_id AND state = 'Online'";
                $count_result = $conn -> query($count_sql);
                
                if($count_result){
                    $count = mysqli_fetch_assoc($count_result);
                    $transit_id = $count['transit_id'];
                    $insert_sql = "INSERT INTO `Coordinates`(trunk_id, latitude, longitude, transit_id) VALUES($trunk_id, -12.8027342, 28.2411498, $transit_id)";
